...
try {
    ...
    if ($action === 'listar') {
        if ($ehAdmin) {
            $st = $pdo->query("SELECT id, aluno_nome, criado_em, atualizado_em FROM intus_relatorio_fotos ORDER BY atualizado_em DESC LIMIT 200");
            $lista = $st->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $st = $pdo->prepare("SELECT id, aluno_nome, criado_em, atualizado_em FROM intus_relatorio_fotos WHERE idprofessor = ? ORDER BY atualizado_em DESC LIMIT 200");
    ...
    if ($action === 'obter') {
        ...
        echo json_encode(['ok' => true, 'relatorio' => [
            'id' => $id, 'aluno_nome' => $rel['aluno_nome'], 'bg' => $rel['bg'], 'marca' => (bool)$rel['marca'],
    ...
    if ($action === 'criar') {
        ...
        $st = $pdo->prepare("INSERT INTO intus_relatorio_fotos (idprofessor, aluno_nome, bg, marca) VALUES (?, ?, ?, ?)");
        $st->execute([$idprofessor, $alunoNome, $bg, $marca]);
    ...
    if ($action === 'salvar_meta') {
        ...
        $marca = $ehAdmin ? (empty($body['marca']) ? 0 : 1) : 1;
        $st = $pdo->prepare("UPDATE intus_relatorio_fotos SET aluno_nome = ?, bg = ?, marca = ? WHERE id = ?");
        $st->execute([$alunoNome, $bg, $marca, (int)$rel['id']]);
    ...
    if ($action === 'salvar_linha') {
        ...
        foreach (['esquerda', 'direita'] as $lado) {
            ...
            $campos[$lado] = [$tag, $arquivo, $zoom, $panX, $panY];
        }

        if ($existente) {
            $sql = "UPDATE intus_relatorio_fotos_linha SET label=?, largura=?, altura=?,
                    esquerda_tag=?, esquerda_arquivo=?, esquerda_zoom=?, esquerda_pan_x=?, esquerda_pan_y=?,
                    direita_tag=?, direita_arquivo=?, direita_zoom=?, direita_pan_x=?, direita_pan_y=?
                    WHERE id = ?";
